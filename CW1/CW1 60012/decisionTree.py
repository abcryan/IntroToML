...
class Node:
    ...
    def calculate_final_x(self, mod_sum):
        # calculate final x positions by shifting nodes by calculated amount
        self.X += mod_sum
        mod_sum += self.mod
        if self.left:
    ...
    def calculate_init_x(self, node_size, sibling_distance):
        ...
        # if it's a leaf
        if self.is_leaf:
            # if not the left node, set X to left sibling + spacing
            if not self.is_leftmost:
                self.X = self.parent.left.X + node_size + sibling_distance
            else:
                self.X = 0
        # has two children, position it in between the children
        else:
            midpoint = (self.left.X + self.right.X) / 2
            if self.is_leftmost:
                self.X = midpoint
            # if this node is on the right, possibly need to shift it and its children 
            else:
                self.X = self.parent.left.X + node_size + sibling_distance
                self.mod = self.X - midpoint
        
        if not self.is_leaf and (not self.is_leftmost):
            # if the node has children, check if subtrees overlap and shift accordingly
    ...
    def check_conflicts(self, node_size, sibling_distance):
        ...
        # only for right nodes, gather right contour of left sibling and calculate distance needed to shift entire subtree
        if not self.is_leftmost:
...
def decision_tree_learning(training_dataset, depth=0):
    # get all of the labels from the samples
    unique_labels = np.unique(training_dataset[:, -1])
    # case 1: only one label is in this dataset -> this is a leaf
    if len(unique_labels) == 1:
        return (Node(attribute=None, value=None, left=None, right=None, label=unique_labels[0], is_leaf=True, depth=depth), depth)
    
    # case 2: more than one label remaining -> continue learning/splitting
